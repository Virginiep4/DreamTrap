...
public class Character extends Entities {
	...
	public void updatePos() {
		...
		if (HelpMethods.CanMoveHere(posX + (int)xSpeed, posY +(int)ySpeed, width, height, lvlData)) {
	...
	public void jumpAnimation() {
		...
		// ascending phase for the first half of jump phases
		      ...
		      /*  // Appliquer la gravité
		        if (jumpingPhase < MAX_JUMP_PHASE) {
		            // Phase de montée
		            if (jumpingPhase < (MAX_JUMP_PHASE / 2)) {
		                this.posY -= airSpeed; // Déplacer vers le haut
		                airSpeed -= GRAVITY; // Appliquer la gravité
		            } else {
		                // Phase de descente
		                this.posY += airSpeed; // Déplacer vers le bas
		                airSpeed += GRAVITY; // Appliquer la gravité
		            }
		            jumpingPhase++;
		        } else {
	...
	public void updateVerticalMovement() {
	    ...
	    if (jumping) {
	        ...
	        int tileYPos = (int)HelpMethods.GetEntityYPosUnderRoofOrAboveFloor(new Rectangle2D.Float(posX, posY, width, height), airSpeed);
	        if (posY >= tileYPos) {

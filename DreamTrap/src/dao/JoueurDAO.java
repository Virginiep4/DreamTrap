...
public class JoueurDAO extends DAO {
	...
	public Character find(String playerName) {
		...
		String req = "SELECT * FROM joueur J JOIN avoir A ON J.idnom = A.idnom INNER JOIN progression P ON A.idprogression = P.idprogression WHERE J.nom LIKE ?";
		this.open(req);
		try {
			...
			if (rs.next()) {
				...
				// check if we have a load already and so if we got a select * from joueur where
				// OIN avoir A ON J.idnom = A.idnom INNER JOIN progression P ON A.idprogression
				// = P.idprogression WHERE win=1
